...
import { Globe, RotateCcw, Trash2, Loader2 } from "lucide-react";
...
export function RunbookActions({ runbookId, status }: Props) {
  ...
  async function performAction(action: string) {
    ...
    try {
      if (action === "delete") {
        const res = await fetch(`/api/runbooks/${runbookId}`, {
          method: "DELETE",
        });
  ...
  return (
    <div className="space-y-2">
      ...
      <button
        onClick={() => {
          if (confirm("Are you sure you want to delete this runbook? This action cannot be undone.")) {
